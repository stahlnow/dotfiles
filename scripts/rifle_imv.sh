...
if [ $# -eq 0 ]; then
	# just open the first image in the current directory
	path=$(find -L . -maxdepth 1 -type f -iregex '.*\(jpe?g\|bmp\|tif\|tiff\|png\|gif\|svg\)$' -print | sort | head -1)
	if [ "$path" == "" ]; then
...
listfiles () {
    find -L "$(dirname "$target")" -maxdepth 1 -type f -iregex \
      '.*\(jpe?g\|bmp\|tif\|tiff\|png\|gif\|svg\)$' -print0 | sort -z
}
...
if [ -n "$count" ]; then
    listfiles | xargs -0 imv -d -s shrink -n "$count" --
else
    imv -d -s shrink -- "$@" # fallback
fi
