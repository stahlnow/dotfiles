-- artificial general intelligence (lol just kidding)
	...
	config = function()
		...
		gp.setup({
			...
			hooks = {
				...
				UnitTests = function(_gp, params)
					local template = "I have the following code from {{filename}}:\n\n"
						.. "```{{filetype}}\n{{selection}}\n```\n\n"
						.. "Please respond by writing unit tests for the code above using pytest."
					local agent = _gp.get_command_agent()
					_gp.Prompt(params, _gp.Target.enew, agent, template)
				end,

				UnitTestsParametrized = function(_gp, params)
					local template = "I have the following code from {{filename}}:\n\n"
						.. "```{{filetype}}\n{{selection}}\n```\n\n"
