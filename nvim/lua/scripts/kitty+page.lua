-- kitty scrollback_pager with nvim
-- set in 'kitty.conf'
...
-- changed mappings to my workflow (q for visual mode, y will yank AND quit, space+q will quit)
...
return function(INPUT_LINE_NUMBER)
    ...
    -- Map 'q' to visual block mode
    vim.api.nvim_buf_set_keymap(term_buf, 'n', 'q', '<c-V>', { })

    -- Map 'y' to first yank the visual selection (if any), which makes the copy selection work, and then quit.
    vim.api.nvim_buf_set_keymap(term_buf, 'v', 'y', 'y<Cmd>qa!<CR>', { })

    -- Map '<Leader>q' to quit
    vim.api.nvim_buf_set_keymap(term_buf, 'n', '<SPACE>q', '<Cmd>qa!<CR>', { })
